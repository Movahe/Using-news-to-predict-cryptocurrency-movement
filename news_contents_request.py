...
class GetData:
    ...
    def parse_page_cryptocurrencynews(self, html):
        ...
        news_list = []
        for _, n in enumerate(news):
            # if index != 1:
            #     continue
            # title = n.find('a', rel="bookmark").text[1:]
            date = n.find('span', {"class": "entry-meta-date updated"}).text
            url_inner_next = n.find('a', rel="bookmark").get("href")
            # print(url_inner_next)
            r = requests.get(url_inner_next)
            soup_in_bowl = BeautifulSoup(r.text, 'html.parser')
            article = soup_in_bowl.find('div', class_="entry-content clearfix")
            # print(article)
    ...
    def parse_page_cryptonews(self, html):
        driver = webdriver.Chrome("/Users/Shiyong/Downloads/chromedriver")
        driver.get(self.url)
        ...
        news_list = []
        for n in news:
            url_inner_link = "https://cryptonews.com" + n.find('h4').find("a").get("href")
            # title = n.find("h4").find('a').text
            date = n.find('time').get("datetime")[:10]
            r = requests.get(url_inner_link)
            soup_in_bowl = BeautifulSoup(r.text, 'html.parser')
            article = soup_in_bowl.find('div', class_="content")
            # print(article)
            # exit()
            paragraphs = article.find_all('p')
            full_article = ""
...
def main():
    url = "https://cryptonews.com/news/bitcoin-news/"     #nickname: cryptonews# Major challenge: No "next page", only "load more".
